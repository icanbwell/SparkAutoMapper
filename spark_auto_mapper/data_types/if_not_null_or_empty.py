...
class AutoMapperIfNotNullOrEmptyDataType(
    ...
    def get_column_spec(self, source_df: Optional[DataFrame], current_column: Optional[Column], parent_columns: Optional[List[Column]]) -> Column:
        column_spec = when(
            self.check.get_column_spec(source_df=source_df, current_column=current_column, parent_columns=parent_columns).isNull()
            | self.check.get_column_spec(source_df=source_df, current_column=current_column,
                                         parent_columns=parent_columns).eqNullSafe(""),
            self.when_null_or_empty.get_column_spec(source_df=source_df, current_column=current_column,
