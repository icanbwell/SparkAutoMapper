...
def test_auto_mapper_base64(spark_session: SparkSession) -> None:
    ...
    # Act
    mapper = AutoMapper(
        view="content", source_view="responses", keys=["id"]
    ).columns(encoded_column=A.base64(A.column("data")))
    ...
    assert_compare_expressions(
        sql_expressions["encoded_column"],
        base64(col("b.data")).alias("encoded_column")
    )
    ...
    assert result_df.where("id == 1").select(
        "encoded_column"
    ).collect()[0][0] == "VGhpcyBpcyBkYXRhIDE="
    assert result_df.where("id == 2").select(
        "encoded_column"
    ).collect()[0][0] == "VGhpcyBpcyBkYXRhIDI="
