...
def test_automapper_if_not(spark_session: SparkSession) -> None:
    ...
    mapper = AutoMapper(
        ...
        age=A.if_not(
            column=A.column("my_age"), check=A.text("55"), value=A.number(A.text("100"))
        )
    )
    ...
    assert str(sql_expressions["age"]) == str(
        when(col("b.my_age").eqNullSafe(lit("55").cast(StringType())), lit(None))
    ...
    assert result_df.where("member_id == 1").select("age").collect()[0][0] == 100
