...
def test_automapper_map(spark_session: SparkSession) -> None:
    ...
    mapper = AutoMapper(
        ...
        has_kids=A.map(
            A.column("has_kids"), {"Y": "Yes", "N": "No"}, A.column("has_kids")
        ),
        short_col_name=A.map(
            A.column("has_kids"), {"Y": "Yes", "N": "No"}, "[has_kids]"
        ),
        lit_col=A.map(A.column("has_kids"), {"Y": "Yes", "N": "No"}, "TRUE"),
        blank_col=A.map(A.column("has_kids"), {"Y": "Yes", "N": "No"}, ""),
    )
