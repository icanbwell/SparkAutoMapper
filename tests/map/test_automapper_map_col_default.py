...
def test_automapper_map(spark_session: SparkSession) -> None:
    ...
    assert str(sql_expressions["has_kids2"]) == str(
        when(col("b.has_kids").eqNullSafe(
            lit("Y")
        ), lit("Yes")).when(col("b.has_kids").eqNullSafe(lit("N")),
                            lit("No")).otherwise(col("b.has_kids")
    ...
    assert str(sql_expressions["has_kids3"]) == str(
        when(col("b.has_kids").eqNullSafe(
            lit("Y")
        ), lit("Yes")).when(col("b.has_kids").eqNullSafe(lit("N")),
                            lit("No")).otherwise(lit(123.3)
                                                 ).alias("has_kids3")
    )
    ...
    assert result_df.where("member_id == 3").select("has_kids2"
                                                    ).collect()[0][0] == "f"
    assert result_df.where("member_id == 3"
                           ).select("has_kids3").collect()[0][0] == "123.3"
