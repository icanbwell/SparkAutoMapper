...
def test_automapper_optional(spark_session: SparkSession) -> None:
    ...
    mapper = AutoMapper(
        ...
        optional_age=AutoMapperOptionalType(
            column=A.number(A.column("my_age")), default=A.text("my_age")
        ),
        optional_foo=AutoMapperOptionalType(
            column=A.column("foo"), default=A.text("foo")
        ),
    )
...
def test_automapper_optional_ifexists(spark_session: SparkSession) -> None:
    ...
    mapper = AutoMapper(
        ...
        optional_age=AutoMapperOptionalType(
            column=A.number(A.column("my_age")),
            default=A.text("my_age"),
            if_exists=A.text("age col is there"),
        ),
        optional_foo=AutoMapperOptionalType(
            column=A.column("foo"),
            default=A.text("foo"),
            if_exists=A.text("foo exists"),
        ),
    )
    ...
    assert result_df.where("member_id == 1").select(
        "optional_age", "optional_foo"
    ).collect()[0][:] == ("age col is there", "foo")
    assert result_df.where("member_id == 2").select(
        "optional_age", "optional_foo"
    ).collect()[0][:] == ("age col is there", "foo")
