...
def test_auto_mapper_substring_by_delimiter(spark_session: SparkSession) -> None:
    # Arrange
    ...
    ).columns(my_column=A.substring_by_delimiter(A.column("last_name"), "s", 1))
    ...
    assert (
        result_df.where("member_id == 1").select("my_column").collect()[0][0] == "Qure"
    )
    assert (
